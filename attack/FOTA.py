...
class FOTA(Attack):
    ...
    def __init__(self, model, eps=8/255, alpha=2/255, steps=5, decay=1.0, tau=1.0, kappa1=1.0, kappa2=1.0, num_classes=100, correct_logit_mean_std=None, wrong_logit_mean_std=None):
        super().__init__("FOTA", model)
        self.eps = eps
        self.steps = steps
        self.decay = decay
        self.alpha = alpha
    ...
    def forward_for_RR(self, images, labels, restart, lambda1=1.0, lambda2=0.5):
        ...
        wrong_logit_mean = torch.zeros(len(images), self.num_classes).to(self.device) + self.wrong_logit_mean_std[:, 0]
        wrong_logit_std = torch.zeros(len(images), self.num_classes).to(self.device) + self.wrong_logit_mean_std[:, 1]
        correct_logit_mean = torch.zeros(len(images), self.num_classes).to(self.device) + self.correct_logit_mean_std[:,
                                                                                          0]
        correct_logit_std = torch.zeros(len(images), self.num_classes).to(self.device) + self.correct_logit_mean_std[:,
                                                                                         1]
